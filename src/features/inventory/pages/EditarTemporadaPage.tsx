import { IconAlertCircle, IconArrowLeft, IconCalendar, IconRefresh } from '@tabler/icons-react'
...
export function EditarTemporadaPage() {
  ...
  useEffect(() => {
    if (temporada) {
      reset({ nombre: temporada.nombre, activo: temporada.activo })
    }
  }, [temporada, reset])
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-[#3d3b4f]">Editar temporada</h1>
          </div>
          <p className="mt-1.5 text-sm text-[#6e6b7b]">Nombre y visibilidad en catálogos de artículos.</p>
        </div>
        {backLink}
      </div>
